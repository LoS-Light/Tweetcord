    ...
    async def get_home_html(self):
        ...
        try:
            response = await self._session.request(
                method="GET", url="https://x.com/?mx=2", headers=headers
            )

            if response.status_code not in range(200, 300):
                response = await self._session.request(
                    method="GET", url=self._builder.URL_HOME_PAGE, headers=headers
                )
            ...
            migration_redirection_url = (
                re.search(MIGRATION_REGEX, str(migration_url))
                or re.search(MIGRATION_REGEX, str(response.content))
            )
            ...
            # NEW: X stopped embedding ondemand.s on /?mx=2 (~2026-06-23).
            # /home still contains the webpack chunk map needed for transaction IDs.
